...
from image_search import call_image_search
...
def parse_args():
    ...
    parser.add_argument('--image_root', type=str, 
                        default="xxx/data/Twitter-GMNER/whole_image",
    ...
    parser.add_argument('--round_1_prompt', type=str,
                        default="xxx/prompts/Twitter-GMNER-Prompt/round_1_user_prompt_1.txt",
                        help='Path to round 1 user prompt file')
    parser.add_argument('--after_text_search_prompt', type=str,
                        default="xxx/prompts/Twitter-GMNER-Prompt/after_text_search_prompt_1.txt",
                        help='Path to after text search prompt file')
    parser.add_argument('--after_image_search_prompt', type=str,
                        default="xxx/prompts/Twitter-GMNER-Prompt/after_image_search_prompt_1.txt",
...
def run_inference(model, processor, sample, image_root, search_cache_path, 
    ...
    response_1 = generate_response(model, processor, messages, max_new_tokens)
    ...
    if text_search_content:
        ...
        if queries:
            ...
            response_2 = generate_response(model, processor, messages, max_new_tokens)
    ...
    if queries:
        ...
        response_3 = generate_response(model, processor, messages, max_new_tokens)
    ...
    for idx in range(start_idx, end_idx):
        ...
        try:
            final_response, original_size, processed_size = run_inference(
                model, processor, sample, args.image_root, args.search_cache_path,
                round_1_prompt, after_text_search_prompt, after_image_search_prompt,
                args.max_new_tokens
            )
