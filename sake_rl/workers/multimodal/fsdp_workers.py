...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if role == 'actor':
            ...
            from sake_rl.utils.torch_functional import get_sigmoid_decay_schedule
            ...
            if lr_sigmoid_decay:
                ...
                print(f'[sake_rl LR Sigmoid Decay] {lr_sigmoid_decay_ratio=} {num_warmup_steps=}')
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
            ...
            if self.config.rollout.use_fire_sampling:
                # FIXME: use_fire_sampling does not support vLLMRollout_MultiTurn_SAKE_RL
                from verl.workers.rollout.vllm_rollout import (
                    FIREvLLMRollout as vLLMRollout,
                )
            else:
                from sake_rl.workers.multimodal.rollout.vllm_rollout_spmd import vLLMRollout_MultiTurn_SAKE_RL
            from verl.workers.sharding_manager.fsdp_vllm import FSDPVLLMShardingManager

            log_gpu_memory_usage('Before building vllm_multi_turn rollout', logger=None)
            local_path = copy_to_local(self.config.model.path)
            if vllm_mode == 'spmd':
                rollout = vLLMRollout_MultiTurn_SAKE_RL(
                    model_path=local_path,
                    config=self.config.rollout,
                    tokenizer=self.tokenizer,
                    model_hf_config=self.actor_model_config,
    ...
    def init_model(self):
        from sake_rl.workers.multimodal.actor.dp_actor import MmDataParallelPPOActor
